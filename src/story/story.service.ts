...
export class StoryService {
  ...
  async getStories(filter: {
    ...
    if (filter.kidId) {
     ...
     // Age filter with priority: ageRange > age > kid.ageGroup
    if (filter.ageRange) {
      const ageGroup = await this.ageService.findGroupForRange(filter.ageRange);
      where.ageMin = { lte: ageGroup.max };
      where.ageMax = { gte: ageGroup.min };
    } else if (filter.age && !targetLevel && !where.ageMin) {
      where.ageMin = { lte: filter.age };
      where.ageMax = { gte: filter.age };
    } else if (kid.ageGroup && !targetLevel && !where.ageMin) {
      where.ageMin = { lte: kid.ageGroup.max };
      where.ageMax = { gte: kid.ageGroup.min };
    }
...
return {
  data: stories.map((story) => ({
    ...
    ageGroup: story.ageGroup
      ...
      ? { min: story.ageGroup.min, max: story.ageGroup.max }
      : { min: story.ageMin, max: story.ageMax },
  })),
  pagination: {
    currentPage: page,
    totalPages,
