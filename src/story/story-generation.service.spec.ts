...
describe('StoryGenerationService', () => {
    ...
    const mockGeneratedStory: GeneratedStory = {
        ...
        content: 'Once upon a time there was a brave hero who went on an adventure.',
        questions: [{ question: 'Who was brave?', options: ['Hero', 'Villain'], answer: 0 }],
        theme: ['Adventure'],
        category: ['Fantasy'],
        ageMin: 4,
        ageMax: 8,
    ...
    const mockCreatedStory = {
        ...
        title: 'The Adventure Begins',
        description: 'A tale of bravery',
        textContent: 'Once upon a time there was a brave hero who went on an adventure.',
    ...
    beforeEach(async () => {
        mockStoryRepository = {
            ...
            findKidById: jest.fn().mockResolvedValue({ id: 'kid-123', parentId: 'parent-456' }),
            findAllThemes: jest.fn().mockResolvedValue([{ id: 'theme-1', name: 'Adventure' }]),
            findAllCategories: jest.fn().mockResolvedValue([{ id: 'cat-1', name: 'Fantasy' }]),
            executeTransaction: jest.fn().mockImplementation((fn) => fn({
                story: {
    ...
    describe('generateStoryWithAI', () => {
        ...
        it('should resolve season IDs to names if provided', async () => {
            ...
            expect(mockStoryRepository.findSeasonsByIds).toHaveBeenCalledWith(['season-1']);
    ...
    describe('generateStoryForKid', () => {
        it('should generate a personalized story for a kid', async () => {
            ...
            expect(mockStoryRepository.findKidByIdWithPreferences).toHaveBeenCalledWith('kid-123');
            expect(mockGeminiService.generateStory).toHaveBeenCalled();
            expect(result).toEqual(mockCreatedStory);
        });

        it('should throw NotFoundException if kid not found', async () => {
            (mockStoryRepository.findKidByIdWithPreferences as jest.Mock).mockResolvedValue(null);
        ...
        it('should parse age range from kid profile', async () => {
            (mockStoryRepository.findKidByIdWithPreferences as jest.Mock).mockResolvedValue({
                ...mockKid,
                ageRange: '5-7',
            });
    ...
    describe('persistGeneratedStory (via generateStoryWithAI)', () => {
        it('should create story with calculated word count and duration', async () => {
            ...
            const transactionFn = (mockStoryRepository.executeTransaction as jest.Mock).mock.calls[0][0];
            expect(transactionFn).toBeDefined();
        });

        it('should continue without audio if TTS fails', async () => {
            (mockTextToSpeechService.synthesizeStory as jest.Mock).mockRejectedValue(new Error('TTS failed'));
