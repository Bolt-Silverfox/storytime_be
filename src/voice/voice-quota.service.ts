...
export class VoiceQuotaService {
    ...
    async canUseVoice(userId: string, voiceId: string): Promise<boolean> {
        ...
        const defaultVoice = FREE_TIER_LIMITS.VOICES.DEFAULT_VOICE;

        // Check if it's the default voice (by VoiceType enum or elevenLabsId)
        if (voiceId === defaultVoice) return true;
        const defaultVoiceConfig = VOICE_CONFIG[defaultVoice];
        if (defaultVoiceConfig && voiceId === defaultVoiceConfig.elevenLabsId) return true;
    ...
    async setSecondVoice(userId: string, voiceId: string): Promise<void> {
        const isPremium = await this.isPremiumUser(userId);
        if (isPremium) {
            throw new BadRequestException('Premium users have unlimited voice access and do not need to set a second voice');
        }
