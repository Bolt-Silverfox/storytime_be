...
describe('SpeechToTextService', () => {
  ...
  beforeEach(async () => {
    ...
    elevenLabsProvider = module.get<ElevenLabsSTTProvider>(
  ...
  describe('transcribeAudio', () => {
    ...
    it('should fallback to Deepgram if ElevenLabs fails', async () => {
      mockElevenLabsTranscribe.mockRejectedValue(
        new Error('ElevenLabs failure'),
      );
    ...
    it('should throw error if both providers fail', async () => {
      ...
      await expect(service.transcribeAudio(buffer, mimetype)).rejects.toThrow(
        'Speech to text failed on both providers',
      );
    });
  });
});
