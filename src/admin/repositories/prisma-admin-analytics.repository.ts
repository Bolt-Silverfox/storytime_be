...
export class PrismaAdminAnalyticsRepository implements IAdminAnalyticsRepository {
    ...
    async getDashboardStats(): Promise<DashboardStatsDto> {
        ...
        const countBetween = (modelName: any, start: Date, end: Date): Promise<number> =>
        ...
        return {
            ...
            performanceMetrics: {
                ...
                newUsers: DashboardUtil.calculateTrend(newUsersThisMonth, prevNewUsersThisMonth, TrendLabel.VS_LAST_MONTH),
    ...
    async getUserGrowth(dateRange: DateRangeDto): Promise<UserGrowthDto[]> {
        ...
        let totalUsers = await this.prisma.user.count({ where: { createdAt: { lt: startDate }, isDeleted: false } });
    ...
    async getStoryStats(): Promise<StoryStatsDto> {
        const [
            ...
            this.prisma.story.count({ where: { isDeleted: false } }),
            this.prisma.story.count({ where: { isDeleted: false } }),
            this.prisma.story.count({ where: { aiGenerated: true, isDeleted: false } }),
            this.prisma.story.count({ where: { recommended: true, isDeleted: false } }),
    ...
    async getContentBreakdown(): Promise<ContentBreakdownDto> {
        const [languageStats, categoryStats, themeStats] = await Promise.all([
            this.prisma.story.groupBy({ by: ['language'], where: { isDeleted: false }, _count: true }),
        ...
        const ageGroups = ageGroupStats.map((stat: any) => ({
            ...
            count: typeof stat._count === 'number' ? stat._count : (stat._count?._all || 0),
        }));

        return {
            byLanguage: languageStats.map((stat) => ({ language: stat.language, count: stat._count })),
            byAgeGroup: ageGroups,
            byCategory: categoryStats.map((cat: any) => ({ categoryName: cat.name, count: cat._count.stories })),
            byTheme: themeStats.map((theme: any) => ({ themeName: theme.name, count: theme._count.stories })),
    ...
    async getSubscriptionAnalytics(dateRange?: DateRangeDto): Promise<SubscriptionAnalyticsDto> {
        ...
        return {
            subscriptionGrowth: subscriptions.map(sub => ({ date: sub.startedAt.toISOString().split('T')[0], count: sub._count })),
            revenueGrowth: revenue.map(rev => ({ date: rev.createdAt.toISOString().split('T')[0], amount: rev._sum.amount || 0 })),
            planBreakdown: planBreakdown.map(plan => ({ plan: plan.plan, count: plan._count })),
            churnRate,
        };
    }
    ...
    private async calculateChurnRate(startDate: Date, endDate: Date): Promise<number> {
        ...
        return totalSubscriptionsAtStart > 0 ? (churnedSubscriptions / totalSubscriptionsAtStart) * 100 : 0;
    }
    ...
    async getRevenueAnalytics(dateRange?: DateRangeDto): Promise<RevenueAnalyticsDto> {
        ...
        return {
            ...
            monthlyRevenue: Array.from(monthlyRevenueMap.entries()).map(([month, total]) => ({ month, total_amount: total })),
            yearlyRevenue: Array.from(yearlyRevenueMap.entries()).map(([year, total]) => ({ year, total_amount: total })),
