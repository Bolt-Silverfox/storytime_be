        ...
        try {
            ...
            for (const category of categories) {
                await this.prisma.category.upsert({
                    ...
                    const existing = await this.prisma.category.findFirst({ where: { name: category.name } });
                    if (existing) {
                        return this.prisma.category.update({
                            where: { id: existing.id },
                            data: { image: category.image, description: category.description, isDeleted: false, deletedAt: null }
                        });
                    }
                    return this.prisma.category.create({ data: { name: category.name, image: category.image, description: category.description } });
                });
            }
            ...
            for (const theme of themes) {
                ...
                if (existing) {
                    await this.prisma.theme.update({
                        where: { id: existing.id },
                        data: { image: theme.image, description: theme.description, isDeleted: false, deletedAt: null }
                    });
                } else {
                    await this.prisma.theme.create({ data: { name: theme.name, image: theme.image, description: theme.description } });
                }
            }

            // Age Groups
            for (const ageGroup of defaultAgeGroups) {
                const existing = await this.prisma.ageGroup.findFirst({ where: { name: ageGroup.name } });
                if (existing) {
                    await this.prisma.ageGroup.update({
                        where: { id: existing.id },
                        data: { min: ageGroup.min, max: ageGroup.max, isDeleted: false, deletedAt: null }
                    });
                } else {
                    await this.prisma.ageGroup.create({ data: { name: ageGroup.name, min: ageGroup.min, max: ageGroup.max } });
                }
            }

            // System Avatars
            for (const avatarData of systemAvatars) {
                const existing = await this.prisma.avatar.findFirst({ where: { name: avatarData.name, isSystemAvatar: true } });
                if (existing) {
                    await this.prisma.avatar.update({
                        where: { id: existing.id },
                        data: { url: avatarData.url, isSystemAvatar: true, isDeleted: false, deletedAt: null }
                    });
                } else {
                    await this.prisma.avatar.create({ data: { name: avatarData.name, url: avatarData.url, isSystemAvatar: true } });
                }
            }
