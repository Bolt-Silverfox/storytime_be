...
describe('AdminService', () => {
    ...
    describe('getDashboardStats', () => {
        it('should return aggregated dashboard statistics', async () => {
            // Mock data
            const mockUsers = [
                { id: '1', subscriptions: [{ status: 'active', endsAt: new Date(Date.now() + 10000) }] }, // Paid
            ...
            prisma.paymentTransaction.aggregate.mockResolvedValue({ _sum: { amount: 5000 } });
            prisma.screenTimeSession.findMany.mockResolvedValue([{ duration: 10 }, { duration: 20 }]);
    ...
    describe('getUserGrowth', () => {
        it('should return user growth stats grouped by date', async () => {
            ...
            const mockUsers = [
                ...
                { createdAt: new Date('2023-01-01T12:00:00Z'), subscriptions: [{ status: 'active' }] },
    ...
    describe('User Management', () => {
            ...
            const mockUsers = [
                {
                    ...
                    subscriptions: [{ id: 'sub-1', status: 'active', endsAt: new Date(Date.now() + 10000) }],
                    _count: { kids: 2, auth: 5, parentFavorites: 3, subscriptions: 1, paymentTransactions: 4 },
                }
            ];
        ...
        it('getUserById: should return detailed user info', async () => {
            ...
            const mockUser = {
                id: userId,
                email: 'test@example.com',
                subscriptions: [{ status: 'active', endsAt: new Date(Date.now() + 10000) }],
                _count: { auth: 10, parentFavorites: 5, voices: 1, subscriptions: 2, supportTickets: 0, paymentTransactions: 5 },
            };

            prisma.user.findUnique.mockResolvedValue(mockUser);
            prisma.paymentTransaction.aggregate.mockResolvedValue({ _sum: { amount: 100 } });
        ...
        it('getUserById: should throw NotFoundException if user does not exist', async () => {
            ...
            await expect(service.getUserById('bad-id')).rejects.toThrow('User with ID bad-id not found');
        });

        it('updateUser: should update user fields', async () => {
            const userId = 'user-1';
            const updateDto = { name: 'New Name' };
            prisma.user.findUnique.mockResolvedValue({ id: userId, email: 'old@example.com' });
