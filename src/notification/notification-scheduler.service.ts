...
export class NotificationSchedulerService {
   ...
   *
   * Integration divergence: develop injected a shared `RedisService`; integration
   * has none, so the lock is taken on a dedicated ioredis client built from
   * REDIS_URL (see notification-redis.provider.ts). The SET NX EX semantics are
   * identical.
   */
