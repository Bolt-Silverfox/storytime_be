...
/**
 * Owns notification-preference (settings) management: CRUD, per-category
 * toggles, grouped views, bulk updates, seeding and soft-delete/restore.
 *
 * Extracted verbatim from NotificationService to keep that class a thin facade;
 * behavior is intentionally identical (direct Prisma access preserved so the
 * exact query semantics — including soft-delete filters — are unchanged).
 */
@Injectable()
export class NotificationSettingsService {
  ...
  async bulkUpdate(
    ...
    const results =
      await this.notificationPreferenceRepository.bulkUpdateEnabledInTransaction(
        dtos.map((dto) => ({ id: dto.id, enabled: dto.enabled })),
      );
  ...
  async updateUserPreferences(
    ...
    const items = categories.flatMap((category) => {
      ...
      return channels.map((type) => ({ userId, category, type, enabled }));
    });
    await this.notificationPreferenceRepository.upsertManyInTransaction(items);
  ...
  async delete(id: string, permanent: boolean = false): Promise<void> {
    ...
    if (permanent) {
      ...
      await this.notificationPreferenceRepository.updateNotificationPreferenceById(
        id,
        {
          isDeleted: true,
          deletedAt: new Date(),
