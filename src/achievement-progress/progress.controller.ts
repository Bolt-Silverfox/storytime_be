...
export class ProgressController {
    ...
    @ApiOperation({
        ...
        description: 'Aggregates streak, badge preview, and progress stats in one call',
    })
    @ApiResponse({
        status: 200,
    ...
    async getHomeScreenData(@Req() req: AuthenticatedRequest): Promise<ProgressHomeResponseDto> {
    ...
    @ApiResponse({ status: 401, description: 'Unauthorized' })
    @HttpCode(HttpStatus.OK)
    async getStreakSummary(@Req() req: AuthenticatedRequest): Promise<StreakResponseDto> {
    ...
    @ApiResponse({ status: 401, description: 'Unauthorized' })
    @HttpCode(HttpStatus.OK)
    async getBadgePreview(@Req() req: AuthenticatedRequest): Promise<BadgePreviewDto[]> {
    ...
    @ApiResponse({ status: 401, description: 'Unauthorized' })
    @HttpCode(HttpStatus.OK)
    async getFullBadgeList(@Req() req: AuthenticatedRequest): Promise<FullBadgeListResponseDto> {
    ...
    @ApiResponse({ status: 401, description: 'Unauthorized' })
    @HttpCode(HttpStatus.OK)
    async getProgressOverview(@Req() req: AuthenticatedRequest): Promise<ProgressOverviewResponseDto> {
