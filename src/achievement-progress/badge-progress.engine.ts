...
export class BadgeProgressEngine implements OnModuleInit {
    ...
    async handleQuizAnswered(event: BadgeEvent & { isCorrect: boolean }) {
        this.logger.log(`Quiz answered event: ${event.userId}, correct: ${event.isCorrect}`);
        await this.badgeService.updateBadgeProgress(
            event.userId,
            'quiz_answered',
            1,
 ...
 */
    ...
    handleBadgeUnlocked(payload: { userId: string; badgeId: string; timestamp: Date }) {
        this.logger.log(`Badge unlocked: ${payload.badgeId} for user ${payload.userId}`);
