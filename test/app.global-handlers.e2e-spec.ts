...
describe('Global Handlers (e2e)', () => {
    ...
    const expectErrorFormat = (res: request.Response, status: number, errorTitle: string) => {
    ...
    describe('SuccessResponseInterceptor', () => {
        it('should wrap successful GET request data into the standard success format', async () => {
            ...
            const res = await request(app.getHttpServer()).get('/api/v1/auth/test-success');
            if (res.status === 200) {
                ...
                console.warn('Skipping SuccessInterceptor test: Could not find /api/v1/auth/test-success endpoint.');
            }
        });
    });
    ...
    describe('HttpExceptionFilter', () => {
        it('should catch unhandled 404s and format them correctly', async () => {
            const res = await request(app.getHttpServer()).get('/api/v1/a-non-existent-route-999');

            expectErrorFormat(res, 404, 'Not Found');
            expect(res.body.message).toEqual('Cannot GET /api/v1/a-non-existent-route-999');
        });
        ...
        it('should format custom thrown HttpException (409 Conflict)', async () => {
            ...
            expectErrorFormat({ status: mockConflictResponse.status, body: mockConflictResponse.body } as any, 409, 'Conflict');
        });
    });
